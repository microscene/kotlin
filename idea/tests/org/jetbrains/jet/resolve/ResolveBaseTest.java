...
public class ResolveBaseTest extends LightCodeInsightTestCase {
    ...
    protected static void doMultiResolveTest() {
        ...
        List<String> resolvedToActual = Lists.newArrayList();
        for (ResolveResult result : results) {
            PsiElement resolvedToElement = result.getElement();
            assertNotNull(resolvedToElement);

            resolvedToActual.add(ReferenceUtils.renderAsGotoImplementation(resolvedToElement));
        }

        assertOrderedEquals(Ordering.natural().sortedCopy(resolvedToActual), Ordering.natural().sortedCopy(expectedReferences));
    }
